...
def blog_create():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(blg)
        db.session.commit()
        current_app.logger.info("Administrator created a new article: "+title)
        return redirect(url_for('blog_bp.index'))
    else:
        flash('No empty form!', 'danger')
        return render_template('backend/createBlog.html', form=form)
...
def blog_content_edit(blog_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        current_app.logger.info("Administrator edit the blog: "+blog.title)
...
def delete(blog_id):
    ...
    blog.state = state
    db.session.commit()
    current_app.logger.info("Administrator deleted the blog: "+ blog.title)
...
def recover(blog_id):
    ...
    blog.state = state
    db.session.commit()
    current_app.logger.info("Administrator recovered the blog: "+ blog.title)
    flash('Successfully recovered!', 'success')
    return redirect(url_for('.blog_edit'))
...
def upload():
    ...
    current_app.logger.info("Administrator uploaded a new file: "+filename)
